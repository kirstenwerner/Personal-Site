...
export default class extends React.Component {
  ...
  validateMail() {
    ...
    if (!this.state.name || this.state.name.length < 3) {
      errors.name =
        "I'd love it if your name was at least 3 characters long...";
    ...
    if (!this.state.email || this.state.email.length < 3) {
      errors.email_1 =
        "Your email address has got to have at least 3 characters.";
  ...
  sendMessage(event) {
    ...
    emailjs
      .send("gmail", "portfoliositecontact", templateParams, `${EmailJSID}`)
      .then(
        function(response) {
